// bridge.js (Smart Cached Version - Final + Quota Tracking)
...
async function searchForStream(lang) {
  ...
  for (const type of eventTypes) {
    ...
    quotaUsed += 100; // track search.list cost
...
async function getVideoStatus(videoId) {
  ...
  quotaUsed += 1; // track videos.list cost
...
async function pollLivestream() {
  ...
  for (const lang of Object.keys(languages)) {
    ...
    if (state === "live") {
      ...
      if (!lockLogged[lang]) {
        console.log(`🔒 Locked ${lang} stream with video id → ${streamCache[lang]}`);
