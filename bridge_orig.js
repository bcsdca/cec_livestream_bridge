// bridge.js (Smart Cached + Mutual Disable + Auto Exit)
...
async function searchForStream(lang) {
  ...
  const eventTypes = ["upcoming", "live"]; // upcoming first
...
async function pollLivestream() {
  ...
  for (const lang of Object.keys(languages)) {
    ...
    if (state === "live") {
      ...
      if (!lockLogged[lang]) {
        console.log(`🔒 Locked ${lang}'s youtube video ID → ${streamCache[lang]}`);
  ...
  /* 🔥 Disable Joint if English + Mandarin have both ever gone live */
  if (
    englishEverLive &&
    mandarinEverLive &&
    !streamEnded["joint"]
  ) {
    console.log("🛑 English & Mandarin have both been live. Joint disabled.");
    streamEnded["joint"] = true;
    streamCache["joint"] = null;
  }

  console.log("Quota used so far:", quotaUsed);
  console.log("----");

  /* 🔥 Auto exit after all streams finished (not at startup) */
  if (hasStarted) {
